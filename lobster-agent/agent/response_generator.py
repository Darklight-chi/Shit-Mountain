"""Generate final reply — uses OpenClaw / OpenAI-compatible LLM."""

import os
from openai import OpenAI
from config.settings import OPENAI_API_KEY, OPENAI_BASE_URL, OPENAI_MODEL
from config.prompts import SYSTEM_PROMPTS, RESPONSE_GENERATION_PROMPT
...
def generate_reply(
    ...
    # For greetings, skip LLM call
    if intent == "general_greeting":
        if locale == "zh":
            return "您好！我是小龙虾客服，请问有什么可以帮您的？"
        return "Hello! I'm Lobster, your customer service assistant. How can I help you?"

    # If tool already gave a good answer and risk is low, use it directly
    if tool_results and risk_level == "low" and intent != "fallback":
        return tool_results

    # Build prompt
    history_text = "\n".join(
        [f"{m['role']}: {m['content']}" for m in history[-5:]]) if history else "无"

    language = "Chinese" if locale == "zh" else "English"
    user_prompt = RESPONSE_GENERATION_PROMPT.format(
        locale=locale, intent=intent, risk_level=risk_level,
        tool_results=tool_results or "无",
        history=history_text, message=message, language=language,
    )
    ...
    try:
        ...
        # Fallback: return tool results or a generic message
        if tool_results:
            return tool_results
        if locale == "zh":
